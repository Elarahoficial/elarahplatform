...
OUT = sys.argv[1] if len(sys.argv) > 1 else "/tmp/claude-0/-home-user-elarahplatform/9abf7e9a-5852-5ed9-badc-3da0f14e2577/scratchpad/Amigas_final.pdf"
...
put(p1, "Arte entre", FB, 37, CORAL_RGB, 70, 342)      # big display headline fills the void the '20' left
put(p1, "amigas", FB, 37, CORAL_RGB, 70, 385)
put(p1, "vem pintar com a gente", FI, 20, DARKt, 70.9, 420)
p1.draw_rect(fitz.Rect(70.9, 441, 116, 443.2), color=None, fill=CORAL_RGB)   # short accent rule
BODY = ('<p>Uma experiência de pintura pra reunir as amigas: '
        '<b style="color:#FF5E8A">leve, criativa e cheia de estilo</b>. '
        'Cada uma escolhe o que quer pintar — taça, tela, aquarela e mais —, relaxa, se diverte '
        'e leva pra casa a sua peça autoral. '
...
TIERS = [
    dict(name="Básico", tag="A experiência, em sua essência.", price="189", more=None,
         bullets=["Pintura em taça, tela, aquarela e mais", "Todos os materiais inclusos",
...
d[1].apply_redactions(images=fitz.PDF_REDACT_IMAGE_NONE, graphics=fitz.PDF_REDACT_LINE_ART_NONE,
                      text=fitz.PDF_REDACT_TEXT_REMOVE)
put(d[1], "Taça ou Tela", FBI, 27, CORAL_RGB, 187.9, 156.2)
# p2 intro
retext(d[1], (52, 171, 505, 250),
       "Aqui cada amiga vira artista por um dia: pinta, personaliza e leva para casa a própria "
       "peça autoral. Uma experiência criativa, relaxante e cheia de estilo — perfeita pra reunir "
       "as amigas e desacelerar do dia a dia. "
...
put_spaced(d[1], "✦ MAIS OPÇÕES", 53.8, 292, 9, CORAL_RGB, FSB)
put(d[1], "Prefere outra ", FB, 18, (0.118, 0.086, 0.098), 53.8, 316)
put(d[1], "pintura?", FI, 18, CORAL_RGB, 53.8+FB.text_length("Prefere outra ", 18), 316)
d[1].insert_textbox(fitz.Rect(53.8, 321, 548, 344),
    "A taça e a tela são só o começo — dá pra escolher também qualquer uma "
...
retext(d[2], (52, 171, 505, 230),
       "A experiência de pintura (taça, tela, aquarela e mais) com toda a estrutura e os mimos. "
...
put(d[1], "Não é só um encontro — é arte, taça e boas risadas.", FI, 12.8, CREAM, 100.7, 658.5)
put(d[1], "Uma tarde que vira memória entre amigas.", FB, 12.8, GOLD, 100.7, 678.0)
...
jp.insert_textbox(fitz.Rect(54, 170, 541, 226),
    ...
    "desacelerar, criar junto e curtir uma manhã diferente com as amigas.",
...
JTIERS = [
    dict(name="Básico", price="239", tag="A experiência, ao ar livre.", kind="plain", more=None,
         bullets=["Pintura em taça, tela ou aquarela", "Materiais + facilitadora", "Cada uma leva a sua peça"]),
