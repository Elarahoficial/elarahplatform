-- =============================================================
-- ELARAH — financial_by_supplier v8: comissão padrão 20% do valor cheio
-- -------------------------------------------------------------
-- Substitui a v6/v7. A comissão (Receita Elarah) por fornecedor:
--   - comissao_type = 'percent' cadastrado → % do VALOR CHEIO
--   - comissao_type = 'fixed'   cadastrado → valor fixo (centavos)
--   - SEM cadastro (caso normal)           → PADRÃO ELARAH = 20% do
--                                             valor cheio (ex: 20% de
--                                             R$150 = R$30)
--
-- Contexto: a parceira cadastra só o VALOR CHEIO e o % do PARCEIRO
-- (repasse). A comissão da Elarah NÃO vai no formulário — é fixa em 20%.
-- Antes a comissão saía como residual "cheio − repasse" (30% = R$45),
-- errado. Agora o padrão é 20% (R$30), batendo com a lista de reservas.
--
